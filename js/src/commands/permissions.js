    ...
    execute: async function({ client, message, args }) {
        ...
        collector.on('end', async (collected) => {
            ...
            if (perms.content.toLowerCase() === 'clear') {
                guildInfo = await client.DBGuild.findByIdAndUpdate(message.guild.id, { $unset: { [`commandPerms.${command.name}`]: 1 } }, { new: true, upsert: true, setDefaultsOnInsert: true });
            } else {
                ...
                guildInfo = await client.DBGuild.findByIdAndUpdate(message.guild.id, { $set: update }, { new: true, upsert: true, setDefaultsOnInsert: true });
            }
