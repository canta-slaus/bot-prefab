...
export default {
  ...
  usage:
    "- `PREFIXpermissions [command]` to display and change the permissions someone needs to use this command.",
  ...
  async execute(client, message, args) {
    ...
    if (!commandPerms || !commandPerms[command.name]) {
      if (command.perms && command.perms.length !== 0)
        ...
        embed.setDescription(
          "You don't need any permissions to run this command."
        );
    } else {
    ...
    collector.on("end", async (collected) => {
      if (collected.size === 0)
        return message.channel.send(
          `${message.author.username}, sorry, if you want to change the permissions, run the command again and react in time.`
        );
      ...
      text +=
        "Reply with the permissions that you want users to have in order to use this command, ";
      text +=
        "e.g.: `cd2` If you want them to have the permissions to kick members, ban members and manage roles in order to use this command.\n";
      text += "Reply with `clear` to reset permissions.";

      embed.setFooter("").setDescription(text);
      message.channel.send(embed);

      const perms = await getReply(message);
      if (!perms)
        return message.channel.send(
          `${message.author.username}, sorry, time is up!`
        );
      ...
      if (perms.content.toLowerCase() === "clear") {
        ...
        if (!permsRegEx.test(perms.content))
          return message.channel.send(
            `${message.author.username}, sorry, that isn't a valid permission string.`
          );
        ...
        guildInfo.commandPerms = guildInfo.commandPerms
      ...
      message.channel.send(
        `${message.author.username}, the permissions for ${command.name} have been overwritten.`
      );
    });
  },
};
