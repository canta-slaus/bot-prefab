  ...
  async execute(client, message, args) {
    if (!prefixRegExp.test(args[0]))
      return message.channel.send(
        `${message.author.username}, that prefix doesn't follow the rules. Please try again.`
      );

    const guildInfo = client.guildInfoCache.get(message.guild.id);
    if (guildInfo.prefix === args[0])
      return message.channel.send(
        `${message.author.username}, please make sure to enter a new prefix.`
      );
    ...
    message.channel.send(
      `${message.author.username}, the new prefix is: \`${args[0]}\``
    );
  },
};
