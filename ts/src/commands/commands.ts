    ...
    if (!args[0]) {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          disabledCommands!.length === 0
            ? "There are no disabled commands in this server!"
      ...
      if (!command)
        return message.channel.send(
          //@ts-ignore
          `The command \`${args[1]}\` does not exist.`
        );

      if (command.canNotDisable)
        return message.channel.send(
          `The command \`${command.name}\` can not be disabled/enabled.`
        );
      ...
      switch (args[0]) {
        case "disable":
          if (disabledCommands!.includes(command.name))
            return message.channel.send(
              `The command \`${command.name}\` is already disabled.`
            );
          ...
          message.channel.send(
            `The command \`${command.name}\` has been disabled.`
          );
          break;
        case "enable":
          if (!disabledCommands!.includes(command.name))
            return message.channel.send(
              `The command \`${command.name}\` is already enabled.`
            );
          ...
          message.channel.send(
            `The command \`${command.name}\` has been enabled.`
          );
          break;
      }
    }
